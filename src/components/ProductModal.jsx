...
const ProductModal = forwardRef((props, ref) => { 
    ...
    function add(keyName) {
      ...
      if ( !foundFlavor ) {
        setCartItems((prev) => [...prev, {name: props.name, price: price, image: props.image[1] }])
      } else {
        setCartItems(
          cartItems.map((items) => {
    ...
    return (
      ...
      <>
        <div className="overlay" onClick={ (event) => {
          ...
          <div className="modal">
              <section className='modal-images' style={ {backgroundImage: `url('${props.image[imageIndex]}')` } }>
                  <div className="previous-image" onClick={() => {
                    ( imageIndex > 0 ) ? setImageIndex( ( imageIndex - 1 ) % 4 ) : setImageIndex(3)
                  }}></div>
                  <div className="next-image" onClick={() => setImageIndex((imageIndex + 1) % 4)}></div>
              </section>
              <section className='modal-info'>
                  ...
                  <div className='controls-container'>
                      ...
                      <div className="quantity-input">
                          <span className="quantity-increase" onClick={() => {
                            quantity == 10 ? setQuantity( quantity ) : setQuantity( quantity + 1 )
                          }}>+</span>
                          <span className="quantity-decrease" onClick={() => {
                            quantity > 0 ? setQuantity( quantity - 1 ) : setQuantity( 0 )
                          }}>-</span>
                          {quantity}
                      </div>
                      <button className='add-to-cart-btn' onClick={() => add(props.name)}>ADD TO CART</button>
